...
if __name__ == '__main__':
	...
	df = pd.DataFrame(index=['SGD', 'Momentum', 'AdaGrad', 'Adam'], columns=['Booth', 'Himmelblau', 'Rosenbrock'])
	steps_df = df.copy()
	df.fillna(0, inplace=True)
	steps_df.fillna(100000, inplace=True)
	while lr < 1:  # test each learning rate configuration
		for func in funcs:
			opts = [opt.SGD(func, lr), opt.Momentum(func, lr), opt.AdaGrad(func, lr), opt.Adam(func, lr)]
			print(f'\n{func.name} , lr={lr}:')
